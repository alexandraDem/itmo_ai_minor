...
- F1 учитывает и **precision**, и **recall**, то есть одновременно штрафует за ложные срабатывания 
  и за пропуски. 
- Макроусреднение даёт **равный вес каждому классу**, даже если распределение по классам  немного неравномерное. Это важно, чтобы, например, модель хорошо различала не только самый   частый символ, но и более редкие. В нашем кейсе может быть дисбаланс классов (люди чаще используют доллары и евро).

Дополнительно будем смотреть на:

- **Accuracy (доля верных ответов)** —  метрика, которая показывает, в какой доле случаев модель вообще не ошиблась. Она досаточно проста для понимания бизнес ценности и легко считается.

В итоге:  
- для подробного анализа и сравнения моделей используем в качестве основной метрики используем **F1**,  
- для общей интуитивной оценки дополнительно — **accuracy**.
